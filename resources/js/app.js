 ...
 */
 ...
 var app = new Vue({
    ...
    methods: {
        ...
        listen: function listen(cid) {
            var _this = this;

            Echo.channel('channel.' + cid).listen('MessagePost', function(msg) {
        ...
        selectChannel: function selectChannel(id = 1) {
            ...
            if (ind >= 0) {
                if(this.selectedChannel.id > 0) {
                    Echo.leave('channel.' + this.selectedChannel.id);
                }

                this.selectedChannel = this.channels[ind];
                this.getMessages();
                this.listen(this.selectedChannel.id);
            }
        },
...
function showLoader(willShow = true) {
    document.getElementById("loader").style.display = willShow ? 'block' : 'none';
}
